function  dat = calcCharge(elements,Twindow)
    if nargin == 0
        %close; 
        clear;
    ...
    [~,time,stimOn, stimOff] = getStimuliData(elements);
    %get baseline 
    preIdx =  cellfun(@(t)t>=timOn-Twindow & t<stimOn,time,'unif',0);
    postIdx = cellfun(@(t)t>=stimOn & t<stimOn + Twindow, time, 'unif',0);
    baseLine = cellfun(@(d)mean(d(preIdx)),dat,'unif',0);
    %
